...
As assembled transcripts are often incomplete, and annotated gene models might miss exons, by default GET_HOMOLOGUES-EST computes alignment coverage with respect to the shortest sequence, as illustrated on __Figure 4__. 
...
We can start by defining an enviromental variable pointing to our GET_HOMOLOGUES installation:
...
#### 3.3.1 Preparing input sequences and extracting CDS from transcripts

The main script takes input sequences in FASTA format, which might be GZIP- or BZIP2-compressed, contained in a folder containing at least two files with extension .fna, one per cultivar/ecotype. Yhould consider adding also sequences from one or more outgroups if you plan to run phylogenomic analyses with the resulting clusters. In either case, nucleotide .fna files might have twin .faa files with translated amino acid sequences in the same order. These you get from resources such as [Ensembl Plants](http://plants.ensembl.org) or [Phytozome](https://phytozome.jgi.doe.gov), or obtain with script *transcripts2cds.pl*, as in the example:
...
Other optional analyses you might want to carry out on your input sequences include computing their coding potential or their gene completeness (see __Note __^[Software choices include CPC2 and RNAsamba [@Kang2017; @Camargo2020], trained on human truth sets, and BUSCO [@Seppey2019]]).

You don't need .faa files to run GET_HOMOLOGUES-EST, but then all the Pfam-based analyses and the phylogenomics recipes of GET_PHYLOMARKERS cannot be used. However, that allows to clusters any kind of nucleotide sequence, such as conserved intergenic regions or transposons.

Note that filenames matching the tag 'flcdna' are handled as full-length transcripts, and this information will be used downstream in order to estimate coverage. By default only sequences of 20 or more bases are considered (see __Note __^[Global variable $MINSEQLENGTH controls the minimum length of sequences to be considered; the default value is 20 but you can edit in the source to increase it.]).

Let's now get all the barley sequences and extract CDS from them:
