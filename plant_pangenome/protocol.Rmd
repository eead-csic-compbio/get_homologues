...
In order to annotate protein domains and to translate open reading frames (ORFs) a couple of databases must be downloaded and formatted, which you can do by calling in the terminal:
...
#### High performance cluster (HPC) configuration
...
GET_HOMOLOGUES-EST has been adapted to the large size of plant genomic data sets, and adds new features to adequately handle redundant and fragmented transcript sequences, as those usually obtained from transcript profiling experiments, as well as incomplete/fragmented gene models from WGS assemblies. Sequence clusters can be produced using the bidirectional best-hit or the OrthoMCL [@Li2003] algorithms.The granularity of the clusters can be tuned by a configurable filtering strategy based on a combination of BLAST pairwise alignment parameters and, optionally, hmmscan-based scanning of Pfam domain composition of the proteins encoded in each cluster. __Figure 3__ summarizes how GET_HOMOLOGUES-EST works.

![](Figures/Figure3.png)

By default redundant sequences are removed from input sets if they overlap a longer sequence over a length $\geq 40$ or when they are completely matched (see __Figure 4__). This is to prevent BLAST results being biased by transcript isoforms.

![](Figures/Figure4.png)

As assembled transcripts are often incomplete, as well as gene models, by default GET_HOMOLOGUES-EST computes alignment coverage with respect to the shortest sequence, as illustrated on __Figure 5__. Users can add the flcdna string to input FASTA files to indicate that coverage should be computed with respect to full-length cDNA sequences, when available. 
...
The mixture model implemented in GET_HOMOLOGUES-EST was first described by @Snipen2009.
...
## Example protocol for plant transcripts
...
<!--
In this section we present a detailed protocol to fit exponential and binomial mixture models to  estimate core and pan-genome sizes, compute pan-genome trees from the pan-genome matrix using a parsimony criterion, analyze and graphically represent the pan-genome structure and identify lineage-specific gene families for  12 complete pIncA/C plasmids available in NCBI RefSeq. 
-->

<!-- Si tienes CDS sequences faa/fna Phytozome/Ensembl salta a -->
<!--!In addition, the user can choose which genome should be used as the reference using option -r.](Figures/Figure3.png) -->
...
The first version of this protocol was funded by Centro de Bioinformática y Biología Computacional de Colombia – [BIOS](http://bios.co) for a two-day workshop organized by Marco Cristancho at Manizales, Colombia, in March 2017.


## Figure captions

**Figure 1.** Pan-gene set of three genomes (A, B and C). In this example there are 101 core loci and 44 (11+9+24) shell loci are found in two accessions. Finally, 82 (35+25+22) cloud loci are annotated only in one genome. The pan-gene set size is 82+44+101=227 loci.

**Figure 2.** Functional annotations (COGs) of core and accessory sequences in two bacterial sets show differences.
