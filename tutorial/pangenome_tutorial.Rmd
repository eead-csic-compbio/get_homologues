---
title: 'From genomes to pangenomes: understanding variation among individuals and
  species'
author:
- affiliation: ARAID & [Estación Experimental de Aula Dei-CSIC](http://www.eead.csic.es/compbio) (Spain) and [UNAM](http://www.ccg.unam.mx/%7Evinuesa) (Mexico)
  name: Bruno Contreras Moreira & Pablo Vinuesa
date: '`r Sys.Date()`'
output:
  html_document:
    fig_caption: yes
...
<!-- por defecto chicago bib style , bibtex generado en www.bioinformatics.org/texmed -->
...
cd soft
wget -c 'https://github.com/eead-csic-compbio/get_homologues/releases/download/v3.0.3/get_homologues-x86_64-20170302.tgz'
tar xvfz get_homologues-x86_64-20170302.tgz
```
...
For this course the following dependencies were also installed:
```{r, engine='bash', eval=FALSE}
...
```
This will create a folder called *get_homologues*.

Note that you might need to install *git* in your system, as explained earlier.
<!--for instance with 
```{r, engine='bash', eval=FALSE}
sudo apt-get install git
```
--->
Note also that the *git* protocol requires to open port 9418, which might be blocked by your firewall.

The disadvantage of this method is that the binary dependencies are not bundled and must be copied from the current release:

```{r, engine='bash', eval=FALSE}
tar xvfz get_homologues-x86_64-20170302.tgz
cp -r get_homologues-x86_64-20170302/bin/* get_homologues/bin/
rm -rf get_homologues-x86_64-20170302 get_homologues-x86_64-20170302.tgz
```

The main advantage of this approach is that future updates are very simple, after moving to the git repository:
```{r, engine='bash', eval=FALSE}
...
## Docker container bundled with GET_PHYLOMARKERS

A way to use GET_HOMOLOGUES with all software dependencies pre-installed is the Docker image available at 
[dockerhub](https://hub.docker.com/r/csicunam/get_homologues). This container includes also [GET_PHYLOMARKERS](https://github.com/vinuesa/get_phylomarkers). The GET_PHYLOMARKERS [manual](https://github.com/vinuesa/get_phylomarkers) explains how to use nucleotide & peptide clusters produced by GET_HOMOLOGUES to compute robust multi-gene and pangenome phylogenies.
...
# set data paths
export top_dir=~/bacteria
export gbk_dir=$top_dir/pIncAC

# download genomes
cd $gbk_dir
perl $GETHOMS/download_genomes_ncbi.pl download.txt

## 1 Compute orthologous gene clusters for pIncA/C plasmids & default settings
...
cd $top_dir
perl $GETHOMS/get_homologues.pl -d pIncAC -n 6

# The script: 
# i) creates a new output directory named pIncAC_homologues/ (blast_dir below)
...
export blast_dir=$top_dir/pIncAC_homologues 
cd $blast_dir
...
# 1.9 which plasmid proteome contains the locus with inparalogues 
# for orthologous cluster 1290_hypothetical_protein.faa?
grep '>' 1290_hypothetical_protein.faa | cut -d\| -f2,3 | sort | uniq -c
...
cd $top_dir
nohup $GETHOMS/get_homologues.pl -d pIncAC -D -n 6 &> log.get_homologues_pIncAC_BDBH_C75D_allTaxa &
...
# Alternatively check the contents of EscherichiacolistrainSCEC2plasmidpSCEC2NC022377_f0_alltaxa_algBDBH_Pfam_e0_.cluster_list
grep -c '>' *faa | grep -v ':12'
...
# 2.5.1 generate two files listing the clusters found by the standard and Pfam-domain filtered BDBH clusters
...
# 2.5.2 find the difference between the two lists
...
cd $top_dir
$GETHOMS/get_homologues.pl -d pIncAC -G -D -t 0 -A -c &> log.get_homologues_pIncAC_GDt0c 
$GETHOMS/get_homologues.pl -d pIncAC -M -D -t 0 -A -c &> log.get_homologues_pIncAC_MDt0c

cat pIncAC_homologues/EscherichiacolistrainSCEC2plasmidpSCEC2NC022377_f0_0taxa_algOMCL_Pfam_e0_Avg_identity.tab
...
cd $blast_dir
$GETHOMS/compare_clusters.pl -d EscherichiacolistrainSCEC2plasmidpSCEC2NC022377_f0_0taxa_algCOG_Pfam_e0_,\
EscherichiacolistrainSCEC2plasmidpSCEC2NC022377_f0_0taxa_algOMCL_Pfam_e0_ -o intersect_pan_CM_Dt0 -t 0 \
-m -T 
...
# 5.1 find out how many plasmids contain NDM-1 genes and check their taxon names
grep '>' 887_blaNDM-1.faa

# (recommended) check the BLAST hits of sequence 887
cd ../..
$GETHOMS/check_BDBHs.pl -d pIncAC_homologues -i 887 -D
