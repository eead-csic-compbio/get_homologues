...
# 2005-12 CCG/UNAM, Mexico, EEAD/CSIC, Zaragoza, Spain
...
@EXPORT = qw(
  ...
  find_taxa_FASTA_headers find_taxa_FASTA_array_headers read_FASTA_file_array run_PARS NAME SEQ
  );
...
sub feature_is_installed
{
  ...
  if($feature eq 'BLAST')
  {
    if($env_missing =~ /BLASTP/ || $env_missing =~ /BLASTN/ || $env_missing =~ /FORMATDB/){ return 0 }
  }
...
sub read_FASTA_file_array
{
  ...
  if($skipamino || $skipidentical)
  {
    ...
    foreach $seq ( 0 .. $n_of_sequences )
    {
      ...
      if($FASTA[$seq][SEQ] =~ /QWERTYIPSDFHKLVNM/i)
      {
...
# Nov2012 to be used when extract_CDSs fails, in cases such as FN869568.gbk
