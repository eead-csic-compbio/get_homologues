...
sub use_specified_genetic_code {
    ...
    if(!$codontables->{$special_code})
    {
        print "# use_specified_genetic_code : unrecognized genetic code table, using 'universal'\n";
	 }
	 else
	 {
         # update codon table	 
         $currentCode = $special_code;
	      ...
	      my ($codon,$aa);
         my $newCodonTable = Bio::Tools::CodonTable -> new ( -id => $currentCode );
         foreach $codon (keys(%codon_table))
         {
             $aa = $newCodonTable->translate($codon); print STDERR "$codon => $aa\n" if $SEE;
