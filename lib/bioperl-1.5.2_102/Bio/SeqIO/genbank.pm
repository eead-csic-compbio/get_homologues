...
sub next_seq {
  ...
  RECORDSTART:
	...
	if( defined ($_) ) {
#	    if( /^CONTIG/o ){
		# Bruno, Jul 2014  http://bioperl.996286.n3.nabble.com/Genbank-files-with-CONTIG-lines-in-them-td17261.html
		# fixed provisionaly at http://cpansearch.perl.org/src/CJFIELDS/BioPerl-1.6.924/Bio/SeqIO/genbank.pm
		#my @contig;
#		my $ctg = ''; # taken from https://metacpan.org/source/CJFIELDS/BioPerl-1.6.900/Bio/SeqIO/genbank.pm
...
#        }
#		# Bruno, Jul2013
#		#fails with perl 5.14.2 (works ok with 5.8.8)
#		#while($_ !~ m{^//}) { # end of file
#		#    $_ =~ /^(?:CONTIG)?\s+(.*)/;
#		#    $annotation->add_Annotation(
...
#		$self->_pushback($_);
#	    } 
		 if( /^WGS|WGS_SCAFLD\s+/o ) { # catch WGS/WGS_SCAFLD lines
...
sub write_seq {
	...
	# Bruno Jul2014 http://bioperl.996286.n3.nabble.com/Genbank-files-with-CONTIG-lines-in-them-td17261.html
