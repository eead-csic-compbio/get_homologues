...
# This script computes a non-redundant pangenome matrix comparing clusters with BLAST
...
my ($INP_matrix,$INP_taxa,$INP_length,$INP_nucleotides,$INP_use_short_sequence,%opts) = ('',1,0,0,0);
my ($INP_identity,$INP_cover,$INP_reference_cover,$INP_reference_identity,$INP_reference_cluster_dir) = (90,75,50,50,'');
my ($n_of_cpus) = ($BLAST_NOCPU);

getopts('hnes:c:r:N:t:m:l:C:S:', \%opts);
...
if(($opts{'h'})||(scalar(keys(%opts))==0))
{
  ...
  print   "-n \t cluster sequences are nucleotides                          (optional, by default expects proteins)\n"; 
  ...
  print   "\nParameters used to match nr clusters to a set of reference sequences:\n"; 
  print   "-r \t add reference clusters contained in directory              (optional, example: -r cross_species_clusters)\n"; 
  print   "-s \t \%sequence identity cutoff for matching reference clusters  (optional, default:$INP_reference_identity)\n";
  print   "-c \t \%alignment coverage cutoff for matching reference clusters (optional, default:$INP_reference_cover,\n";
  print   "   \t                                                               calculates coverage over longest sequence)\n";
  exit(-1);
}
...
if(defined($opts{'r'}))
{
  if(-d $opts{'r'}){ $INP_reference_cluster_dir = $opts{'r'} }
...
printf("\n# %s -m %s -t %d -l %d -C %d -S %d -n %s -r %s -c %d -s %d -e %d -N %d\n\n",
	$0,$INP_matrix,$INP_taxa,$INP_length,$INP_cover,$INP_identity,$INP_nucleotides,
  $INP_reference_cluster_dir,$INP_reference_cover,$INP_reference_identity,$INP_use_short_sequence,
  $n_of_cpus);
...
## 6) if required match nr clusters to external, pre-built reference clusters
if($INP_reference_cluster_dir)
{
  my ($ref_cluster_id,$id,$n_of_ref_sequences);
  my (@ref_clusterfiles,%ref_length,%ref_seen,@ref_cluster_size); #%ref_cover
  ...
  # save all reference sequences in a single FASTA file to be scanned with BLAST  
  ...
  if(!-s $nr_pangenome_reference_blast_file)
  {
    ...
    { 
      ...
      $command = format_BLASTN_command($nr_pangenome_tmp_fasta_file,$nr_pangenome_reference_blast_file,
        $nr_pangenome_reference_fasta_file,$BLAST_PVALUE_CUTOFF_DEFAULT,2);
    }
    else
    { 
      ...
      {
        die "# EXIT: cannot format BLAST protein sequence base $nr_pangenome_reference_fasta_file\n";
      }
  ...
  while(<BPO>)
  {
    ...
    #($hit,$col,$querylen,$pSid,$subjectlen,$pEvalue,$ppercID,$simspan,$pbits) = split(/;/,$_);
    ...
    if($cover >= $INP_reference_cover && $ppercID >= $INP_reference_identity) 
    {
      ...
      #$ref_cover{$col}{$ref_cluster_id} = $cover; # in case coverage was further needed
...
print <<'TRANS';
  perl -ane '$r++;for(1 .. @F){$m[$r][$_]=$F[$_-1]};$mx=@F;END{for(1 .. $mx){for $t(1 .. $r){print"$m[$t][$_]\t"}print"\n"}}' \
TRANS
